...
function DiseaseDetector() {
  ...
  // Inside the handleAnalyzeClick function in DiseaseDetector.jsx
  ...
  const handleAnalyzeClick = useCallback(async () => {
    ...
    try {
      ...
      // The backend returns { analysis: "...", filename: "..." }
      // Clean the analysis string to remove Markdown bold markers "**"
      const cleanedAnalysis = result.analysis.replaceAll("*" , ""); // Remove all occurrences of **

      setAnalysisResult(cleanedAnalysis); // Set the cleaned result to state
      // You could also access result.filename here if needed
    } catch (err) {
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 bg-white rounded-lg shadow-md max-w-3xl border border-gray-200">
      ...
      {error && (
        <div
          className="my-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded-md text-center"
          role="alert"
        >
